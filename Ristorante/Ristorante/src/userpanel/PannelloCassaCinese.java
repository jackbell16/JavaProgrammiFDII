...
public class PannelloCassaCinese {
	...
	private static void selezionaPortataMenu(final ProdottoCinese prodotto,final int i) {
		jMenuteItemPortate.get(i).addActionListener(newAction(prodotto, i));
	}

	private static ActionListener newAction(final ProdottoCinese prodotto,
			final int i) {
		return new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				CASSA_CINESE.aggiungiProdotti(prodotto, quantità);
				CASSA_CINESE.getConto().get(i).setAbbondante(setAbbondante);
				
				if(setAbbondante){
					area.append(quantità+" x "+prodotto.toString()+" = "+calcolaRisultatoParziale(prodotto)+ " Euro"+ " Abbondante");
				}else{
					area.append(quantità+" x "+prodotto.toString()+" = "+calcolaRisultatoParziale(prodotto)+ " Euro");
				}
				area.append(newLine);
				setAbbondante=false;
			}
	...
	private static double calcolaRisultatoParziale(final ProdottoCinese prodotto){
		if(setAbbondante){
			return quantità*(prodotto.getPrezzo()+CassaCinese.aggiungaAbbondante);
		}
	...
	private static void impostaSelezionaPizze() {
		for (int i = 0; i < jMenuteItemPortate.size(); i++) {
			...
			jMenuteItemPortate.get(i).addActionListener(newAction(RISTORANTE_CINESE.getPortate().get(i), i));
		}
	}
